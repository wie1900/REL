...
@section('title', 'Expense documents')
...
@section('content')
    ...
    @foreach ($docs as $doc)
        <div class="row border-bottom mt-0 pt-1 rowcolor">
            ...
            <div class="col-2">
                <div class="row mb-1">
                    <div class="col-12 col-md-5 border-end border-1 text-end color">{{ number_format($doc->getTotalPrice(),2,'.','') }}</div>
                    <div class="col-12 col-md-7 border-end border-1 text-center color">
                        <a href="/month/{{ substr($doc->gen,0,7) }}" target="_blank" class="fw-bold">Month</a>
                    </div>
                </div>
            </div>
        </div>
